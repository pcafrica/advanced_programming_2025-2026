...
## Control structures. Functions. Docstrings.
<br>
...
| Memory management | Automatic (Garbage Collector)        | Manual/RAII                   |
| Performance       | Slower                               | Faster                        |
| Compilation       | Interpreted                          | Compiled                      |
| Learning curve    | Gentle                               | Steep                         |
| Use case          | Prototyping, scripting, data science | Systems, performance-critical |
...
- (Advanced users) **Virtual environments:** Use virtual environments like [`venv`](https://docs.python.org/3/library/venv.html) (built-in) or [conda](https://conda.io/projects/conda/en/latest/user-guide/install/index.html) to isolate project dependencies and avoid conflicts between different projects.
...
See the [Python 3 documentation](https://docs.python.org/3.14/library/stdtypes.html) for a summary of the standard built-in Python datatypes.
...
There are three distinct numeric types: `integers`, `floating point numbers`, and `complex numbers`. We can determine the type of an object in Python using `type()`. We can print the value of the object using `print()`. Unlike C++, Python integers have **arbitrary precision**!
...
greeting = "Hello, "
name = "Alice"
full_name = greeting + name  # Concatenation: "Hello, Alice".
```
...
There are *many* string methods. Check out the [documentation](https://docs.python.org/3.14/library/stdtypes.html#string-methods).
...
See format code options [here](https://docs.python.org/3.14/library/string.html#format-specification-mini-language).
...
You can see the documentation for more [list methods](https://docs.python.org/3.14/tutorial/datastructures.html#more-on-lists).
...
[Conditional statements](https://docs.python.org/3.14/tutorial/controlflow.html) allow us to write programs where only certain blocks of code are executed depending on the state of the program. Let's look at some examples and take note of the keywords, syntax and indentation. 
...
Read more in the documentation [here](https://docs.python.org/3.14/library/stdtypes.html#truth-value-testing).
...
We can also use a [`while` loop](https://docs.python.org/3.14/reference/compound_stmts.html#while) to excute a block of code until a condition is met.
...
Nested comprehension (be careful with readability!):
```python
matrix = [[i*j for j in range(3)] for i in range(3)]
# [[0, 0, 0], [0, 1, 2], [0, 2, 4]]
```

Tuple comprehension doesn't work as you might expect: we get a **generator** instead (explained later).
...
A [function](https://docs.python.org/3.14/tutorial/controlflow.html#defining-functions) is a reusable piece of code that can accept input parameters, also known as *arguments*. For example, let's define a function called `square` which takes one input parameter `n` and returns the square `n**2`:
...
[Type hinting](https://docs.python.org/3.14/library/typing.html) is exactly what it sounds like, it hints at the data type of function arguments. You can indicate the type of an argument in a function using the syntax `argument : dtype`, and the type of the return value using `def func() -> dtype`. Let's see an example:
...
- **PEP 8:** Follow Python's style guide for readable code
  - 4 spaces for indentation (not tabs)
  - 79 characters per line (flexible)
  - Use snake_case for functions and variables

- **Zen of Python:** Run `import this` in Python interpreter
  - "Explicit is better than implicit"
  - "Simple is better than complex"
  - "Readability counts"

- **Use meaningful names:** `calculate_average()` not `calc_avg()`

- **Don't reinvent the wheel:** Check standard library first
